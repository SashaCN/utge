...
<div class="wrapper flex-sb product-page">
    ...
    <div class="product-list flex-sb">
        ...
        @endphp
        <a href="{{ route('product', ['id' => $product->id, 'price' => $min_price], $product->localization[0]) }}">
            <figure class="product product_id shadow-box flex-col {{ $available }}" data-product-id="{{ $product->id }}">
                ...
                <figcaption>
                    ...
                    @endif
                    <p class="description active-size">@if($product->mass_id == 1)@lang('admin.massa_neto'): @endif{{ $product->sizeprices->where('price', $min_price)->first()->size }}</p>
        ...
        @foreach ($_REQUEST as $key => $id)
            ...
            @if (explode('_', $key)[0] == 'subcategoryid')
                @foreach ($subcategories->where('id', $id) as $subcategory)
                    @if (isset($subcategory->localization[0]->$locale) && $subcategory->localization[0]->$locale != 'utge undefined description')
                        <div class="text-wrap shadow-box">
                            <p>{!! $subcategory->localization[0]->$locale !!}</p>
                        </div>
                    @endif
                @endforeach
            @endif
