<?php

return [
    'utge_admin' => 'UTGE: Админ панель',
    'products' => 'Товары',
    'categories' => 'Категории',
    'orders' => 'Заказы',
    'news' => 'Список новостей',
    'modules' => 'Модули',
    'add' => 'Добавить',
    'product' => 'Товар',
    'service' => 'Услугу',
    'new' => 'Новость',
    'category' => 'Категория',
    'image' => 'Картинка',
    'title' => 'Название',
    'description' => 'Описание',
    'filters' => 'Фильтры',
    'price' => 'Цена',
    'sizeprice' => 'Вес/Цена | Наличие',
    'another' => 'Другое',
    'photo' => 'Фото',
    'action' => 'Действие',
    'product_add_category' => 'Добавить категорию товаров',
    'product_category_change' => 'Редактировать категорию товаров',
    'choose_type' => 'Выберите к какому виду продуктов относится категория',
    'choose_category' => 'Выберите к какой категории относится подкатегория',
    'product_add_subcategory' => 'Добавить подкатегорию товаров',
    'product_list' => 'Список товаров',
    'product_change' => 'Редактировать товар',
    'product_create' => 'Добавить товар',
    'sub_category_add' => 'Подкатегорию',
    'product_type' => 'Вид товара',
    'product_category_list' => 'Список категорий товаров',
    'product_subcategory_list' => 'Список подкатегорий товаров',
    'product_type_list' => 'Список видов товаров',
    'add_uk_title' => 'Введите название на украинском языке',
    'add_ru_title' => 'Введите название на русском языке',
    'add_uk_desc' => 'Введите описание на украинском языке',
    'add_ru_desc' => 'Введите описание на русском языке',
    'add_price' => 'Введите цену (Пример: 10)',
    'add_price_ru' => 'Введите цену на русском (необязательно)',
    'add_price_units' => 'Введите единицы измерения цены (Пример: грн, грн/кг)',
    'add_size' => 'Введите вес (Пример: 100 г)',
    'add_available' => 'Наличие товара',
    'available' => 'В наличии',
    'not_available' => 'Нет в наличии',
    'waiting_available' => 'Ожидается',
    'available_for_order' => 'Под заказ',
    'add_home_view' => 'Отображать на главной странице',
    'home_view' => 'Отображать',
    'not_home_view' => 'Не отображать',
    'add_list_position' => 'Позиция в списке',
    'update-image' => 'Для того чтобы, выбрать новое фото - нажмите на картинку',
    'save-new-phot' => 'Сохранить новое фото',
    'admin.add_sub_category' => 'Добавить подкатегорию',
    'add_product_type' => 'Добавить вид товаров',
    'add_size_price' => 'Добавить вес/цену',
    'delete_size_price' => 'Убрать вес/цену',
    'child_page_father' => 'Выбрать блок',
    'child_page_slider' => 'Выбрать блок слайдера',
    'childPage_create' => 'Добавить блок',
    'childPage_create_slider' => 'Добавить слайд',
    'route' => 'Блок',
    'news_create' => 'Добавить новости',
    'to_sub_category' => 'По подкатегориям',
    'to_category' => 'По категориям',
    'to_product_type' => 'По видам товаров',
    'search' => 'Поиск',
    'childPage_create' => 'Добавить блок',
    'route' => 'Блок',
    'news_create' => 'Добавить новости',
    'error-size' => 'Вес',
    'error-material' => 'Материал',
    'error-price' => 'Цена',
    'error-price_units' => 'Единицы измерения цены',
    'error-available' => 'Наличие товара',
    'choose-block' => 'Выберите сначала блок во вкладке Блок',
    'phone' => 'Блок телефонов',
    'add_phone' => 'Введите номер телефона',
    'this-block-img' => 'В данном блоке картинка не используется',
    'this-block-desc' => 'В данном блоке описание не используется',
    'this-block-name' => 'В данном блоке название не используется',
    'block' => 'Блок',
    'first_create_image' => 'Выберите сначала блок во вкладке Другое',
    'trash' => 'Удаленные товары и услуги',
    'trash_box' => 'Удаленные товары',
    'service_trash_box' => 'Удаленные услуги',
    'seo_panel' => 'SEO панель',
    'add_key_seo_uk' => 'Мета-тег KEYWORDS на украинском языке',
    'add_desc_seo_uk' => 'Мета-тег DESCRIPTION на украинском языке',
    'og_add_title_seo_uk' => 'Мета-тег OG:TITLE на украинском языке',
    'add_title_seo_uk' => 'Мета-тег TITLE на украинском языке',
    'add_key_seo' => 'Мета-тег KEYWORDS на русском языке',
    'add_desc_seo' => 'Мета-тег DESCRIPTION на русском языке',
    'og_add_title_seo' => 'Мета-тег OG:TITLE на русском языке',
    'add_title_seo' => 'Мета-тег TITLE на русском языке',
    'og_add_desc_seo_uk' => 'Мета-тег OG:DESCRIPTION на украинском языке',
    'og_add_desc_seo' => 'Мета-тег OG:DESCRIPTION на русском языке',
    'add_custom_seo_uk' => 'Собственный мета-тег на украинском языке',
    'add_custom_seo' => 'Собственный мета-тег на русском языке',
    'product_subcategory_change' => 'Редактировать подкатегорию товаров',
    'product_type_change' => 'Редактировать вид товаров',
    'components' => 'Компоненты',
    'categories_product' => 'Категории товаров',
    'subcategory_product' => 'Подкатегории товаров',
    'product_types' => 'Виды товаров',
    'category_news' => 'Категории новостей',
    'services_types' => 'Виды услуг',
    'add_services_type' => 'Добавить вид услуг',
    'services_type_change' => 'Редактировать вид услуг',
    'services_type_list' => 'Список видов услуг',
    'services_category' => 'Категории услуг',
    'services' => 'Услуги',
    'services_list' => 'Список услуг',
    'delete_material_price' => 'Убрать материал/цену',
    'add_material_price' => 'Добавить материал/цену',
    'add_units' => 'Единица измирения',
    'add_material' => 'Материал',
    'services_sizeprice' => 'Цена/Материал',
    'services_create' => 'Создать услугу',
    'service_sizeprice' => 'Материал/цена',
    'logo-img' => 'Логотип (изображение)',
    'logo-name' => 'Логотип (текст)',
    'footer-place' => 'Расположение',
    'email' => 'Почта',
    'seo_create' => 'SEO оптимизация',
    'home-page' => 'Главная страница',
    'product-page' => 'Страница товаров',
    'delivery-page' => 'Доставка и оплата',
    'news-page' => 'Страница новостей',
    'contacts-page' => 'Страница контактов',
    'services-page' => 'Страница услуг',
    'page' => 'Выберете страницу',
    'title-page' => 'Название страницы',
    'models-url' => 'Ссылка',
    'slider' => 'Слайдеры',
    'sliderNew' => 'Новый слайдер',
    'models-slider_order' => 'Порядок слайда',
    'services-change' => 'Редактировать услугу',
    'material-price' => 'Материал/Цена',
    'change-services-type' => 'Выберите вид услуги',
    'change-news' => 'Редактировать новость',
    'privacy_policy' => 'Политика конфиденциальности',
    'privacy_policy_order' => 'Порядок',


    'cart' => 'Безналичный расчет',
    'privat' => 'Картой на ПриватБанк',
    'cash' => 'Наличкой',
    'payment' => 'Способ оплаты',
    'avtolux' => 'Автолюкс',
    'intime' => 'Интайм',
    'adres-delivery' => 'Адресс',
    'ind-delivery' => 'Самовывоз',
    'nova-poshta' => 'Новая Почта',
    'ukr-poshta' => 'Укрпочта',
    'adress' => 'Адресс доставки',
    'city' => 'Город',
    'order-product-list' => 'Список заказа',
    'total_price' => 'Общая стоимость заказа',
    'gen-price-unit' => 'Общая цена товара',
    'price-of-unit' => 'Цена товара',
    'service_category_list' => 'Список категорий услуг',
    'service_add_category' => 'Добавить категорию услуг',
    'services_category_change' => 'Редактировать категорию услуг',
    'news_add_category' => 'Добавить категорию новостей',
    'news_category_list' => 'Список категорий новостей',
    'news_category_change' => 'Редактировать категорию новостей',

    'under-certificate' => 'Выбрать новый сертификат',
    'under-type' => 'Выберите вид товара',
    'under-category' => 'Выберите категорию',
    'under-sub-category' => 'Выберите подкатегорию',

    // auntification
    'name' => 'Имя',

    // product page
    'quality-certificate' => 'сертификат качества',
    'number' => '№',
    'fio' => 'Имя/Фамилия',
    'contacts' => 'Контакты',
    'status' => 'Статус заказа',
    'order-list' => 'Список заказов',
    'order-service' => 'Заказ №',
    'interes-service' => 'Что интересует',
    'change' => 'Изменить',
    'save-new-pdf' => 'Сохранить',
    'massa_neto' => 'масса нетто',
    'add_title_materials' => 'Материал на украинском языке',
    'add_title_ru_materials' => 'Материал на русском языке',

    // dashboard

    'dashboard' => 'Панель состояния',
    'goods_count' => 'Количество товаров на сайте',
    'more' => 'Подробнее ...',
    'service_count' => 'Количество услуг на сайте',
    'new_product_orders' => 'Новые заказы товаров',
    'new_service_orders' => 'Новые заказы услуг',
    'total_product_orders' => 'Общее количество заказанных товаров',
    'total_product_orders_annulled' => 'Общее количество аннулированных заказов товаров',
    'total_service_orders' => 'Общее количество заказанных услуг',
    'total_service_orders_annulled' => 'Общее количество аннулированных заказов услуг',
    'total_product_price' => 'Общая стоимость заказов',
    'total_product_price_annulled' => 'Общая стоимость аннулированных заказов',
    'order-annulled' => 'Аннулированный заказ',
    'order-processed' => 'Выполненный заказ',
    'order-in-processing' => 'Заказ в обработке',
    'order-new' => 'Новый заказ',
    'messages' => 'Сообщения',
    'message' => 'Сообщение',
    'message-processed' => 'Обработанное сообщение',
    'message-in-processing' => 'Сообщение в обработке',
    'message-new' => 'Новое сообщение',
    'message-status' => 'Статус сообщения',

    'close' => 'Закрыть',
];

