<?php

    return [
        'utge_admin' => 'UTGE: Адмін панель',
        'products' => 'Товари',
        'categories' => 'Категорії',
        'orders' => 'Замовлення',
        'news' => 'Список новин',
        'modules' => 'Модулі',
        'add' => 'Додати',
        'product' => 'Товар',
        'service' => 'Послугу',
        'new' => 'Новину',
        'category' => 'Категорія',
        'image' => 'Картинка',
        'title' => 'Назва',
        'description' => 'Опис',
        'filters' => 'Фільтри',
        'price' => 'Ціна',
        'sizeprice' => 'Вага/Ціна | Наявність',
        'another' => 'Інше',
        'photo' => 'Фото',
        'action' => 'Дія',
        'sub_category_add' => 'Підкатегорію',
        'product_type' => 'Вид товару',
        'product_change' => 'Редагувати товар',
        'product_type_list' => 'Список видів товарів',
        'product_category_list' => 'Список категорій товарів',
        'product_subcategory_list' => 'Список підкатегорій товарів',
        'product_add_category' => 'Додати категорію товару',
        'choose_type' => 'Оберіть до якого виду продукту буде відноситись категорія',
        'choose_category' => 'Оберіть до якої категорії буде відноситись підкатегорія',
        'product_add_subcategory' => 'Додати підкатегорію товару',
        'product_subcategory_change' => 'Редагувати підкатегорію товарів',
        'product_category_change' => 'Редагувати категорію товарів',
        'product_create' => 'Додати товар',
        'product_list' => 'Список товарів',
        'add_uk_title' => 'Введіть назву на українській мові',
        'add_ru_title' => 'Введіть назву на російській мові',
        'add_uk_desc' => 'Введіть опис на українській мові',
        'add_ru_desc' => 'Введіть опис на російській мові',
        'add_price' => 'Введіть ціну (Приклад: 10)',
        'add_price_ru' => "Введіть ціну на російській мові (необов'язково)",
        'add_price_units' => 'Введіть одиниці вимрювання ціни (Приклад: грн, грн/кг)',
        'add_size' => 'Введіть вагу (Приклад: 100 г)',
        'add_available' => 'Наявність товару',
        'available' => 'В наявності',
        'not_available' => 'Немає в наявності',
        'waiting_available' => 'Очікується',
        'available_for_order' => 'Під замовлення',
        'add_home_view' => 'Відображати на головній сторінці',
        'home_view' => 'Відображати',
        'not_home_view' => 'Не віддображати',
        'add_list_position' => 'Позиція в списку',
        'update-image' => 'Для того щоб, вибрати нове фото - натисніть на картинку',
        'save-new-phot' => 'Зберегти нове фото',
        'admin.add_sub_category' => 'Добавити підкатегорію',
        'add_product_type' => 'Додати вид товару',
        'add_size_price' => 'Додати вагу/ціну',
        'delete_size_price' => 'Забрати вагу/ціну',
        'child_page_father' => 'Обрати блок',
        'child_page_slider' => 'Обрати блок слайдера',
        'childPage_create' => 'Додати блок',
        'childPage_create_slider' => 'Додати слайд',
        'route' => 'Блок',
        'news_create' => 'Додати новини',
        'to_sub_category' => 'За підкатегоріями',
        'to_category' => 'За категоріями',
        'to_product_type' => 'За видами товарів',
        'search' => 'Пошук',
        'childPage_create' => 'Додати блок',
        'route' => 'Блок',
        'news_create' => 'Додати новини',
        'error-size' => 'Вага',
        'error-material' => 'Матеріал',
        'error-price' => 'Ціна',
        'error-price_units' => 'Одиниці вимрювання ціни',
        'error-available' => 'Наявність товару',
        'choose-block' => 'Виберіть спочатку блок у вкладці Блок',
        'phone' => 'Блок телефонів',
        'add_phone' => 'Введіть номер телефону',
        'this-block-img' => 'У даному блоці картинка не використовується',
        'this-block-desc' => 'У даному блоці опис не використовується',
        'this-block-name' => 'У даному блоці назва не використовується',
        'block' => 'Блок',
        'first_create_image' => 'Оберіть спочатку блок у вкладці Інше',
        'trash' => 'Видалені товари та послуги',
        'trash_box' => 'Видалені товари',
        'service_trash_box' => 'Видалені послуги',
        'seo_panel' => 'SEO панель',
        'add_key_seo_uk' => 'Мета-тег KEYWORDS на українській мові',
        'add_desc_seo_uk' => 'Мета-тег DESCRIPTION на українській мові',
        'og_add_title_seo_uk' => 'Мета-тег OG:TITLE на українській мові',
        'add_title_seo_uk' => 'Мета-тег TITLE на українській мові',
        'add_key_seo' => 'Мета-тег KEYWORDS на російській мові',
        'add_desc_seo' => 'Мета-тег DESCRIPTION на російській мові',
        'og_add_title_seo' => 'Мета-тег OG:TITLE на російській мові',
        'add_title_seo' => 'Мета-тег TITLE на російській мові',
        'og_add_desc_seo_uk' => 'Мета-тег OG:DESCRIPTION на українській мові',
        'og_add_desc_seo' => 'Мета-тег OG:DESCRIPTION на російській мові',
        'add_custom_seo_uk' => 'Власний мета-тег на українській мові',
        'add_custom_seo' => 'Власний мета-тег на російській мові',
        'product_type_change' => 'Редагувати вид товарів',
        'components' => 'Компоненти',
        'categories_product' => 'Категорії товарів',
        'subcategory_product' => 'Підкатегорії товарів',
        'product_types' => 'Види товарів',
        'category_news' => 'Категорії новин',
        'services_types' => 'Види послуг',
        'add_services_type' => 'Добавити вид послуг',
        'services_type_change' => 'Редагувати вид послуг',
        'services_type_list' => 'Список видів послуг',
        'services_category' => 'Категорії послуг',
        'services' => 'Послуги',
        'services_list' => 'Список послуг',
        'delete_material_price' => 'Видалити матеріал/ціну',
        'add_material_price' => 'Додати матеріал/ціну',
        'add_units' => 'Одиниця виміру',
        'add_material' => 'Матеріал',
        'services_sizeprice' => 'Ціна/Матеріал',
        'services_create' => 'Створити послугу',
        'service_sizeprice' => 'Матеріал/ціна',
        'logo-img' => 'Логотип (зображення)',
        'logo-name' => 'Логотип (текст)',
        'footer-place' => 'Розташування',
        'email' => 'Пошта',
        'seo_create' => 'SEO оптимизація',
        'home-page' => 'Головна сторінка',
        'product-page' => 'Сторінка товарів',
        'delivery-page' => 'Доставка та оплата',
        'news-page' => 'Сторінка новин',
        'contacts-page' => 'Сторінка контактів',
        'services-page' => 'Сторінка послуг',
        'page' => 'Оберіть сторінку',
        'title-page' => 'Назва сторінки',
        'models-url' => 'Посилання',
        'slider' => 'Слайдери',
        'sliderNew' => 'Новий слайдер',
        'slider_link' => 'Посилання слайдеру',
        'models-slider_order' => 'Порядок слайда',
        'services-change' => 'Редагувати послугу',
        'material-price' => 'Матеріал/Ціна',
        'change-services-type' => 'Виберіть вид послуги',
        'change-news' => 'Редагувати новину',
        'service_category_list' => 'Список категорій послуг',
        'service_add_category' => 'Додати категорію послуг',
        'services_category_change' => 'Редагувати категорію послуг',
        'news_add_category' => 'Додати категорію новин',
        'news_category_list' => 'Список категорій новин',
        'news_category_change' => 'Редагувати категорію новин',

        'under-certificate' => 'Вибрати новий сертифікат',
        'under-type' => 'Виберіть вид товару',
        'under-category' => 'Виберіть категорію',
        'under-sub-category' => 'Виберіть підкатегорію',

        // auntification
        'name' => 'Ім`я',

        'number' => '№',
        'fio' => 'Ім`я/Прізвище',
        'contacts' => 'Контакти',
        'status' => 'Статус замовлення',
        'order-list' => 'Список замовлень',
        'order-service' => 'Замовлення №',
        'interes-service' => 'Що цікавить',
        'change' => 'Змінити',



        'cart' => 'Безготівковий розрахунок',
        'privat' => 'Карткою на ПриватБанк',
        'cash' => 'Готівкою',
        'payment' => 'Спосіб оплати',
        'avtolux' => 'Автолюкс',
        'intime' => 'Інтайм',
        'adres-delivery' => 'Адреса',
        'ind-delivery' => 'Самовивіз',
        'nova-poshta' => 'Нова Пошта',
        'ukr-poshta' => 'Укрпошта',
        'adress' => 'Адреса доставки',
        'city' => 'Місто',
        'order-product-list' => 'Список замовлення',
        'total_price' => 'Загальна вартість замовлення',
        'gen-price-unit' => 'Загальна ціна товару',
        'price-of-unit' => 'Ціна товару',
        'save-new-pdf' => 'Зберегти',
        'massa_neto' => 'маса нетто',
        'add_title_materials' => 'Матеріал на українській мові',
        'add_title_ru_materials' => 'Матеріал на російській мові',

        // dashboard

        'dashboard' => 'Панель стану',
        'goods_count' => 'Кількість товарів на сайті',
        'more' => 'Детальніше ...',
        'service_count' => 'Кількість послуг на сайті',
        'new_product_orders' => 'Нові замовлення товарів',
        'new_service_orders' => 'Нові замовлення послуг',
        'total_product_orders' => 'Загальна кількість замовлених товарів',
        'total_product_orders_annulled' => 'Загальна кількість анульованих замовлень товарів',
        'total_service_orders' => 'Загальна кількість замовлених послуг',
        'total_service_orders_annulled' => 'Загальна кількість анульованих замовлень послуг',
        'total_product_price' => 'Загальна вартість замовлень',
        'total_product_price_annulled' => 'Загальна вартість анульованих замовлень',
        'order-annulled' => 'Анульоване замовлення',
        'order-processed' => 'Виконане замовлення',
        'order-in-processing' => 'Замовлення в обробці',
        'order-new' => 'Нове замовлення',
        'messages' => 'Повідомлення',
        'message' => 'Повідомлення',
        'message-processed' => 'Виконане повідомлення ',
        'message-in-processing' => 'Повідомлення в обробці',
        'message-new' => 'Нове повідомлення',
        'message-status' => 'Статус повідомлення',

        'close' => 'Закрити',
];

